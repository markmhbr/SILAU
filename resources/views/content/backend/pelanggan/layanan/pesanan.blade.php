    ...
    <div class="space-y-8 animate-fadeIn">
        <div class="flex flex-col md:flex-row md:items-end justify-between gap-4">
            <div>
                ...
                <p class="text-sm text-slate-500 dark:text-slate-400 mt-1">Pantau status cucian dan riwayat transaksi Anda
                    secara real-time.</p>
            </div>
        </div>

        <div
            class="bg-cyan-50/50 dark:bg-cyan-950/20 border border-cyan-100 dark:border-cyan-800/50 p-5 rounded-[2rem] flex items-start gap-4 backdrop-blur-sm">
            <div
                class="w-12 h-12 bg-white dark:bg-slate-800 text-cyan-500 rounded-2xl flex items-center justify-center shrink-0 shadow-sm">
            ...
            <div class="text-sm">
                <p class="font-bold text-cyan-900 dark:text-cyan-200 uppercase tracking-widest text-[10px] mb-1">Tips
                    Navigasi</p>
                <p class="text-cyan-700/80 dark:text-cyan-400/80 leading-relaxed">Geser tabel ke samping <span
                        class="md:hidden font-bold text-brand">👉</span> untuk melihat detail harga dan status lengkap.</p>
            </div>
        </div>
        ...
        <div
            ...
            <div class="p-6 md:p-8">
                <div class="overflow-x-auto scrollbar-thin scrollbar-thumb-slate-200 dark:scrollbar-thumb-slate-800">
                    <table id="userLayananTable"
                        class="w-full min-w-[1000px] text-sm text-left border-separate border-spacing-y-3">
                        <thead>
                            <tr
                                class="text-slate-400 dark:text-slate-500 uppercase text-[11px] tracking-[0.15em] font-black">
                        ...
                        <tbody class="divide-y-0">
                            @foreach ($transaksis as $transaksi)
                                <tr
                                    class="group bg-slate-50/50 dark:bg-slate-800/30 hover:bg-white dark:hover:bg-slate-800 transition-all duration-300 shadow-sm hover:shadow-md">
                                    <td
                                        class="px-6 py-5 text-center text-slate-400 font-mono rounded-l-2xl border-y border-l border-transparent group-hover:border-slate-100 dark:group-hover:border-slate-700">
                                    ...
                                    <td
                                        class="px-4 py-5 text-right border-y border-transparent group-hover:border-slate-100 dark:group-hover:border-slate-700 whitespace-nowrap">
                                        @php
                                            $diskonNominal = 0;
                                            if ($transaksi->diskon) {
                                        ...
                                        @endphp
                                        @if ($diskonNominal > 0)
                                            <div class="text-rose-500 font-bold">- Rp
                                                {{ number_format($diskonNominal, 0, ',', '.') }}</div>
                                        @else
                                    ...
                                    <td
                                        class="px-6 py-5 text-center rounded-r-2xl border-y border-r border-transparent group-hover:border-slate-100 dark:group-hover:border-slate-700">
                                        ...
                                        @if(in_array($transaksi->status, ['menuju lokasi penjemputan', 'diambil driver']))
                                        <a href="{{ route('tracking', $transaksi->id) }}"
                                            class="inline-flex items-center justify-center w-10 h-10 rounded-xl bg-blue-500 text-white hover:bg-blue-600 hover:shadow-md transition-all ml-1"
                                            title="Lacak Driver">
                                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
