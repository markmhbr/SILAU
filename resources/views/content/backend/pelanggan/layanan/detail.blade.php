                    ...
                    <div class="p-8">
                        <div
                            ...
                            <div class="space-y-1">
                                ...
                                <span
                                    class="inline-flex px-4 py-1.5 rounded-xl text-[11px] font-black uppercase tracking-wider 
                                {{ $transaksi->status == 'selesai' ? 'bg-emerald-100 dark:bg-emerald-500/10 text-emerald-600' : 'bg-amber-100 dark:bg-amber-500/10 text-amber-600' }}">
                                    {{ $transaksi->status }}
                                </span>
                            </div>
                ...
                @if ($transaksi->metode_pembayaran == 'qris' && $transaksi->status == 'pending')
                    <div
                        ...
                        <div class="p-8 text-center space-y-6">
                            <div
                                class="bg-white p-4 rounded-[2rem] border-2 border-slate-50 inline-block shadow-inner ring-8 ring-slate-50 dark:ring-slate-800/50">
                                <img src="{{ asset('assets/img/Capture.PNG') }}" alt="QRIS"
                                    class="w-44 h-auto mx-auto group-hover:scale-105 transition-transform duration-500">
                            </div>

                            <p class="text-[11px] text-slate-500 leading-relaxed font-medium">
                                Silakan scan QR di atas melalui aplikasi e-wallet Anda. Pastikan nominal sesuai dengan total
                                bayar.
                            </p>

                            <form action="{{ route('pelanggan.layanan.bayar', $transaksi->id) }}" method="POST"
                                enctype="multipart/form-data"
                                class="space-y-4 pt-6 border-t border-slate-100 dark:border-slate-800">
                                @csrf
                                <div class="text-left">
                                    <label
                                        class="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3 text-center">Upload
                                        Bukti Transfer</label>
                                    <label
                                        class="group relative flex flex-col items-center justify-center w-full h-36 border-2 border-dashed border-slate-200 dark:border-slate-700 rounded-[2rem] cursor-pointer hover:border-brand hover:bg-brand/5 transition-all">
                                        <div
                                            class="flex flex-col items-center justify-center transition-transform group-hover:-translate-y-1">
                                            <span class="text-2xl mb-2">📸</span>
                                            <p class="text-[10px] text-slate-400 font-bold group-hover:text-brand">KETUK
                                                UNTUK PILIH FILE</p>
                                        </div>
                                ...
                                <button type="submit"
                                    class="w-full py-4 bg-emerald-500 hover:bg-emerald-600 text-white rounded-2xl font-black shadow-lg shadow-emerald-500/20 transition-all active:scale-95 text-xs uppercase tracking-widest">
