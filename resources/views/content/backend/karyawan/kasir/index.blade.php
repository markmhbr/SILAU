...
<div class="max-w-7xl mx-auto space-y-6 animate-fadeIn">
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
            ...
            <p class="text-sm text-slate-500 dark:text-slate-400 mt-1">Kelola semua pesanan pelanggan (Member & Guest) di sini.</p>
        </div>
    </div>
                ...
                <tbody class="divide-y divide-slate-50 dark:divide-slate-800">
                    ...
                    <tr class="hover:bg-slate-50/50 dark:hover:bg-slate-800/30 transition-colors group">
                        <td class="px-6 py-5">
                            ...
                            <p class="text-[10px] text-slate-400 font-medium">{{ $t->created_at->format('d M Y, H:i') }}</p>
                        </td>
                        <td class="px-6 py-5">
                            @if($t->pelanggan)
                                <div class="flex items-center gap-3">
                                    <div class="w-8 h-8 rounded-full bg-emerald-100 dark:bg-emerald-500/10 text-emerald-600 flex items-center justify-center text-[10px] font-black">MB</div>
                                    <div>
                                        <p class="text-sm font-black text-slate-700 dark:text-slate-200">{{ $t->pelanggan->user->name }}</p>
                                        <p class="text-[10px] text-slate-400">{{ $t->pelanggan->user->email }}</p>
                                    </div>
                                </div>
                            @else
                                <div class="flex items-center gap-3">
                                    <div class="w-8 h-8 rounded-full bg-slate-100 dark:bg-slate-800 text-slate-400 flex items-center justify-center text-[10px] font-black">GS</div>
                                    <p class="text-sm font-bold text-slate-400 italic">Pelanggan Guest</p>
                                </div>
                            @endif
                        </td>
                        <td class="px-6 py-5">
                            <p class="text-sm font-bold text-slate-700 dark:text-slate-200 capitalize">{{ $t->layanan->nama_layanan }}</p>
                            <p class="text-[10px] text-slate-400">{{ $t->berat }} kg</p>
                        </td>
                        <td class="px-6 py-5">
                            <p class="text-sm font-black text-slate-800 dark:text-white">Rp {{ number_format($t->total_harga, 0, ',', '.') }}</p>
                            <p class="text-[10px] uppercase font-bold text-slate-400">{{ $t->metode_pembayaran }}</p>
                        </td>
                        <td class="px-6 py-5">
                            @php
                                $statusMap = [
                                ...
                                $style = $statusMap[$t->status] ?? 'bg-slate-100 text-slate-600';
                            @endphp
                            <span class="px-3 py-1 rounded-lg text-[10px] font-black uppercase tracking-wider {{ $style }}">
                                {{ $t->status }}
                            </span>
                        </td>
                    ...
                    <tr>
                        <td colspan="6" class="px-6 py-20 text-center">
                            <div class="space-y-3">
                                ...
                                <p class="text-slate-400 font-bold">Belum ada transaksi hari ini.</p>
                            </div>
                        </td>
                    </tr>
                    @endforelse
