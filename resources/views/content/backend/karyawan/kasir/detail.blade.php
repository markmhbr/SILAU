    ...
    <div class="max-w-6xl mx-auto space-y-8 animate-fadeIn">
        ...
        <div class="flex flex-col md:flex-row md:items-end justify-between gap-4">
            <div>
                ...
                <p class="text-sm text-slate-500 dark:text-slate-400 mt-1">Order ID:
                    <span
                        class="font-mono text-brand font-bold">#{{ $transaksi->order_id ?? 'TRX-' . $transaksi->id }}</span>
                </p>
            </div>
            <div class="flex gap-2">
                <span
                    class="px-4 py-2 bg-slate-100 dark:bg-slate-800 rounded-xl font-bold text-[10px] uppercase tracking-widest text-slate-500">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div class="lg:col-span-2 space-y-6">
                ...
                <div
                    ...
                    <div class="flex flex-wrap gap-8">
                        <div>
                            <p class="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Estimasi Berat
                                (User)</p>
                            <p class="text-lg font-bold text-slate-700 dark:text-slate-300">
                                {{ $transaksi->estimasi_berat ?? '0' }} KG</p>
                        </div>
                        <div>
                            <p class="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Estimasi Harga
                            </p>
                ...
                <div
                    class="bg-white dark:bg-slate-900 rounded-[2.5rem] p-8 shadow-sm border border-slate-200 dark:border-slate-800 relative overflow-hidden">
                    {{-- Watermark Status --}}
                    @if ($transaksi->berat_aktual)
                        <div
                            class="absolute -right-4 -top-4 bg-emerald-500/10 text-emerald-500 px-8 py-4 rotate-12 font-black uppercase text-xs border border-emerald-500/20">
                            Selesai Timbang
                        </div>
                    @endif
                    ...
                    <div class="flex items-center justify-between mb-8">
                        <div class="flex items-center gap-4">
                            <div
                                class="w-12 h-12 bg-slate-100 dark:bg-slate-800 text-brand rounded-2xl flex items-center justify-center text-xl">
                                🧺</div>
                            <div>
                                <h3 class="font-black text-slate-800 dark:text-white uppercase tracking-wider text-sm">
                                    {{ $transaksi->layanan->nama_layanan }}</h3>
                                <p class="text-xs text-slate-400 font-bold italic">
                                    Berat Aktual: <span class="text-brand">{{ $transaksi->berat_aktual ?? 'Belum diisi' }}
                                        KG</span>
                                </p>
                            </div>
                        </div>
                        <div class="text-right">
                            ...
                            <p class="text-2xl font-black text-brand">Rp
                                {{ number_format($transaksi->harga_final ?? 0, 0, ',', '.') }}</p>
                        </div>
                    </div>
                    ...
                    <div class="space-y-4 border-t border-slate-100 dark:border-slate-800 pt-6">
                        <div class="flex justify-between text-sm">
                            ...
                            <span class="text-slate-800 dark:text-slate-200 font-bold">Rp
                                {{ number_format($transaksi->harga_layanan ?? 0, 0, ',', '.') }}</span>
                        </div>
                        <div class="flex justify-between text-sm">
                        ...
                        <div
                            class="flex justify-between text-sm pt-4 border-t border-dashed border-slate-200 dark:border-slate-700">
                            <span class="text-slate-800 dark:text-white font-black uppercase tracking-tight text-lg">Tagihan
                                yang Harus Dibayar</span>
                            <span class="text-brand font-black text-2xl">Rp
                                {{ number_format($transaksi->harga_final ?? 0, 0, ',', '.') }}</span>
                        </div>
                    </div>
                </div>
            ...
            <div class="space-y-6">
                {{-- PANEL UPDATE STATUS --}}
                <div
                    class="bg-white dark:bg-slate-900 p-8 rounded-[2.5rem] shadow-sm border border-slate-200 dark:border-slate-800">
                    <h4 class="font-black text-slate-800 dark:text-white mb-6 uppercase tracking-widest text-xs">Update
                        Status Kerja</h4>
                    ...
                    <form action="{{ route('karyawan.kasir.status', $transaksi->id) }}" method="POST" class="space-y-4">
                        ...
                        <div>
                            <select name="status"
                                class="w-full px-5 py-3.5 rounded-2xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 outline-none text-sm font-bold text-slate-700 dark:text-slate-200">
                                @foreach (['menunggu penjemputan', 'menunggu diantar', 'diambil driver', 'diterima kasir', 'ditimbang', 'menunggu pembayaran', 'dibayar', 'diproses', 'selesai', 'dibatalkan'] as $st)
                        ...
                        <button type="submit"
                            class="w-full py-4 bg-slate-800 hover:bg-black text-white rounded-2xl font-black text-[10px] uppercase tracking-widest transition-all shadow-lg shadow-slate-200">
                            Simpan Perubahan
                        </button>
                    </form>
                </div>
                ...
                <a href="{{ route('karyawan.kasir.index') }}"
                    class="flex items-center justify-center gap-2 text-xs font-black text-slate-400 hover:text-brand transition-colors uppercase tracking-widest">
