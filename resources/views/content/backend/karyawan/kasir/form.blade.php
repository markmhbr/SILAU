    ...
    <div class="max-w-6xl mx-auto space-y-8 animate-fadeIn">
        ...
        <div class="flex flex-col md:flex-row md:items-end justify-between gap-4">
            <div>
                ...
                <p class="text-sm text-slate-500 dark:text-slate-400 mt-1">Input pesanan masuk dari pelanggan (Member/Guest).
                </p>
            </div>
        </div>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div class="lg:col-span-2 space-y-6">
                    ...
                    <div
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                <label
                                    class="block text-sm font-bold mb-2 text-slate-700 dark:text-slate-300">Layanan</label>
                            ...
                            <div>
                                <label class="block text-sm font-bold mb-2 text-slate-700 dark:text-slate-300">Berat
                                    Aktual</label>
                                <div class="relative">
                                    <input type="number" step="0.01" name="berat_aktual" id="beratInput"
                                        class="w-full px-5 py-3.5 rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 outline-none text-sm"
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6">
                            <div>
                                <label
                                    class="block text-sm font-bold mb-2 text-slate-700 dark:text-slate-300">Diskon</label>
                                <select name="diskon_id" id="diskonSelect"
                                    class="w-full px-5 py-3.5 rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 outline-none text-sm">
                                    <option value="">- Tanpa Diskon -</option>
                                    @foreach ($diskon as $d)
                                        <option value="{{ $d->id }}" data-tipe="{{ $d->tipe }}"
                                            data-nilai="{{ $d->nilai }}" data-min="{{ $d->minimal_transaksi }}">
                                            {{ $d->nama_diskon }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div>
                                <label class="block text-sm font-bold mb-2 text-slate-700 dark:text-slate-300">Metode
                                    Bayar</label>
                        ...
                        <div class="mt-6">
                            <label class="block text-sm font-bold mb-2 text-slate-700 dark:text-slate-300">Catatan Pesanan
                                (Opsional)</label>
                            <textarea name="catatan" rows="2"
                                class="w-full px-5 py-3.5 rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 outline-none text-sm"
                                placeholder="Contoh: Baju putih jangan dicampur..."></textarea>
                        </div>
                    </div>
                </div>
                ...
                <div class="lg:col-span-1">
                    <div class="bg-slate-900 rounded-[2.5rem] p-8 text-white shadow-2xl sticky top-24">
                        ...
                        <div class="mb-8">
                            <span class="text-[10px] text-white/50 uppercase font-black tracking-widest">Total Bayar</span>
                            <h2 id="label-total" class="text-4xl font-black tracking-tighter">Rp 0</h2>
                        </div>

                        <button type="submit"
                            class="w-full py-4 bg-brand hover:bg-brandDark text-white rounded-2xl font-black transition-all active:scale-95 uppercase tracking-widest text-sm">
                            Simpan & Proses
                        </button>
                    </div>
                </div>
