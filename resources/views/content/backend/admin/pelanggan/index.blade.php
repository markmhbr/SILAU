    ...
    <div class="bg-blue-50 dark:bg-blue-900/20 border border-blue-100 dark:border-blue-800 p-4 rounded-2xl flex items-start gap-4">
        ...
        <div class="text-sm text-blue-800 dark:text-blue-300">
            ...
            <div class="flex flex-wrap gap-4">
                <span class="flex items-center gap-1.5"><i class="fas fa-edit text-xs"></i> Gunakan tombol biru untuk edit data.</span>
                <span class="flex items-center gap-1.5"><i class="fas fa-trash text-xs text-rose-500"></i> Gunakan tombol merah untuk menghapus data.</span>
            </div>
        </div>
    </div>
                    ...
                    <tr class="hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors group">
                        ...
                        <td class="px-4 py-4">
                            <div class="flex items-center gap-3">
                                <div class="w-8 h-8 rounded-lg bg-slate-100 dark:bg-slate-700 flex items-center justify-center text-primary-600 font-bold text-xs">
                                    {{ substr($pelanggan->user->name, 0, 1) }}
                                </div>
                                <span class="font-semibold text-slate-900 dark:text-white">{{ $pelanggan->user->name }}</span>
                            </div>
                        </td>
                        <td class="px-4 py-4 font-medium text-slate-600 dark:text-slate-400">
                            <a href="https://wa.me/{{ $pelanggan->no_hp }}" target="_blank" class="hover:text-emerald-500 flex items-center gap-2 transition-colors">
                                <i class="fab fa-whatsapp text-emerald-500"></i> {{ $pelanggan->no_hp }}
                            </a>
                        </td>
                        <td class="px-4 py-4 max-w-xs truncate text-slate-500 dark:text-slate-400">{{ $pelanggan->alamat }}</td>
                        <td class="px-4 py-4">
                            <div class="flex items-center justify-center gap-2">
                                ...
                                <form action="{{ route('admin.pelanggan.destroy', $pelanggan->id) }}" method="POST" class="inline">
                                    @csrf
                                    @method('DELETE')
                                    <button type="button" class="btn-delete w-9 h-9 flex items-center justify-center rounded-xl bg-rose-50 dark:bg-rose-900/30 text-rose-600 dark:text-rose-400 hover:bg-rose-600 hover:text-white transition-all shadow-sm"
                                            title="Hapus Data">
