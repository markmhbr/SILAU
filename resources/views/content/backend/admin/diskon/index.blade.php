...
@section('content')
    <div class="space-y-6">
        ...
        <div
            class="bg-amber-50 dark:bg-amber-900/20 border border-amber-100 dark:border-amber-800 p-4 rounded-2xl flex items-start gap-4">
            <div
                class="w-10 h-10 bg-amber-500 text-white rounded-xl flex items-center justify-center shrink-0 shadow-lg shadow-amber-500/20">
                <i class="fas fa-tags text-sm"></i>
            </div>
            <div class="text-sm text-amber-800 dark:text-amber-300">
                <p class="font-bold mb-1">Panduan Pengelolaan:</p>
                <div class="flex flex-wrap gap-4 text-xs">
                    <span class="flex items-center gap-1.5"><i class="fas fa-check-circle text-emerald-500"></i> Aktifkan
                        diskon agar muncul di transaksi.</span>
                    <span class="flex items-center gap-1.5"><i class="fas fa-edit"></i> Edit data untuk mengubah minimal
                        transaksi.</span>
                    <span class="flex items-center gap-1.5"><i class="fas fa-trash text-rose-500"></i> Diskon yang dihapus
                        tidak bisa dikembalikan.</span>
                </div>
            </div>
        </div>
        ...
        <div
            ...
            <div class="px-6 py-5 border-b border-slate-100 dark:border-slate-700 flex justify-between items-center">
                ...
                <a href="{{ route('admin.diskon.create') }}"
                    class="inline-flex items-center gap-2 bg-amber-500 hover:bg-amber-600 text-white px-4 py-2 rounded-xl text-sm font-semibold transition shadow-md shadow-amber-500/20">
                                ...
                                <tr class="hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors">
                                    ...
                                    <td class="px-4 py-4">
                                        <div class="flex items-center gap-3">
                                            <div
                                                class="w-8 h-8 rounded-lg {{ (int) $d->aktif === 1 ? 'bg-emerald-100 text-emerald-600 dark:bg-emerald-900/30 dark:text-emerald-400' : 'bg-slate-100 text-slate-400 dark:bg-slate-700' }} flex items-center justify-center">
                                                <i class="fas fa-ticket-alt text-xs"></i>
                                            </div>
                                            <span
                                                class="font-bold text-slate-900 dark:text-white">{{ $d->nama_diskon }}</span>
                                        </div>
                                    </td>
                                    ...
                                    <td class="px-4 py-4">
                                        <div class="flex items-center justify-center gap-2">
                                            ...
                                            <form action="{{ route('admin.diskon.toggle', $d->id) }}" method="POST"
                                                ...
                                                @method('PATCH')
                                                <button type="submit"
                                                    class="w-9 h-9 flex items-center justify-center rounded-xl transition-all {{ (int) $d->aktif === 1 ? 'bg-emerald-50 text-emerald-600 hover:bg-emerald-600 hover:text-white dark:bg-emerald-900/30' : 'bg-slate-100 text-slate-400 hover:bg-slate-600 hover:text-white dark:bg-slate-700' }}"
                                                    title="{{ (int) $d->aktif === 1 ? 'Nonaktifkan' : 'Aktifkan' }}">
                                                    <i
                                            ...
                                            <form action="{{ route('admin.diskon.destroy', $d->id) }}" method="POST"
                                                ...
                                                @method('DELETE')
                                                <button type="button"
                                                    class="btn-delete w-9 h-9 flex items-center justify-center rounded-xl bg-rose-50 dark:bg-rose-900/30 text-rose-600 dark:text-rose-400 hover:bg-rose-600 hover:text-white transition-all">
    ...
    <script>
        $(document).ready(function() {
            ...
            $('.dataTables_filter input').addClass(
                'bg-slate-100 dark:bg-slate-700 border-transparent rounded-xl text-sm focus:ring-2 focus:ring-amber-500 outline-none px-4 py-2 w-64 mb-4 transition-all'
                );
