...
<body>
    ...
    <div>
        ...
        <div class="right bold">Rp
            {{ number_format($transaksi->harga_estimasi ?? $transaksi->harga_total, 0, ',', '.') }}</div>
    </div>
    ...
    <div>
        @if ($transaksi->diskon)
            ...
            <div class="right">-Rp
                {{ number_format(($transaksi->harga_estimasi ?? $transaksi->harga_total) - ($transaksi->harga_final ?? $transaksi->harga_setelah_diskon), 0, ',', '.') }}
            </div>
            <div class="line"></div>
        @endif

        <div class="bold">TOTAL BAYAR:</div>
        <div class="right bold" style="font-size: 14px;">Rp
            {{ number_format($transaksi->harga_final ?? ($transaksi->harga_setelah_diskon ?? $transaksi->harga_total), 0, ',', '.') }}
        </div>
    ...
    <script>
        window.onload = function() {
            ...
            window.onafterprint = function() {
                // Return to previous page or close window
                window.close();
            };
        }
    </script>
