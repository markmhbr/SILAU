...
@section('content')
    <div class="space-y-6 animate-fadeIn">
        ...
        {{-- INFO PANEL --}}
        <div
            class="bg-blue-50 dark:bg-blue-900/20 border border-blue-100 dark:border-blue-800 p-4 rounded-3xl flex items-start gap-4">
            <div
                class="w-10 h-10 bg-blue-600 text-white rounded-2xl flex items-center justify-center shrink-0 shadow-lg shadow-blue-500/30">
                <i class="fas fa-info-circle"></i>
            </div>
            <div class="text-sm text-blue-800 dark:text-blue-300">
                <p class="font-black mb-1 italic uppercase tracking-wider">Quick Guide:</p>
                <p class="opacity-80">Klik pada label status untuk mengubah progres cucian. Transaksi dengan <span
                        class="font-bold underline">ID merah</span> adalah transaksi yang belum lunas atau butuh perhatian.
                </p>
            </div>
        </div>
                    ...
                    <table id="transaksiTable" class="w-full text-sm text-left border-separate border-spacing-y-3">
                        ...
                        <tbody>
                            @foreach ($transaksis as $transaksi)
                                <tr
                                    class="bg-slate-50/50 dark:bg-slate-700/30 hover:bg-slate-100 dark:hover:bg-slate-700/60 transition-all group">
                                    <td
                                        class="px-4 py-4 text-center font-bold text-slate-400 rounded-l-2xl group-hover:text-brand">
                                        {{ $loop->iteration }}</td>
                                    <td class="px-4 py-4">
                                        <div class="font-mono font-black text-brand tracking-tighter">
                                            {{ $transaksi->order_id }}</div>
                                    ...
                                    <td class="px-4 py-4">
                                        ...
                                        <div
                                            class="text-[9px] px-2 py-0.5 bg-slate-200 dark:bg-slate-600 rounded inline-block text-slate-600 dark:text-slate-300 font-bold uppercase">
                                            {{ $transaksi->layanan->jenis_layanan }}</div>
                                    </td>
                                    ...
                                    <td class="px-4 py-4 text-center">
                                        ...
                                        <div class="relative inline-block text-left group/status">
                                            <button type="button"
                                                class="px-3 py-1.5 rounded-xl text-[9px] font-black uppercase tracking-widest shadow-lg transition-all hover:scale-105 {{ $statusBadge }}">
                                                {{ $transaksi->status }}
                                            </button>
                                    ...
                                    <td class="px-4 py-4 text-center rounded-r-2xl">
                                        <div class="flex items-center justify-center gap-2">
                                            ...
                                            <form action="{{ route('admin.transaksi.destroy', $transaksi->id) }}"
                                                ...
                                                @method('DELETE')
                                                <button type="button"
                                                    class="btn-delete w-9 h-9 flex items-center justify-center rounded-xl bg-rose-50 text-rose-500 hover:bg-rose-500 hover:text-white transition-all shadow-sm">
