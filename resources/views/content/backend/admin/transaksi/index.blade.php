...
@section('content')
    <div class="space-y-6">
        ...
        <div
            class="bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 p-4 rounded-2xl flex items-start gap-4 shadow-sm">
            ...
            <div class="text-sm text-slate-600 dark:text-slate-300">
                ...
                <div class="flex flex-wrap gap-4 text-xs">
                    ...
                    <span class="flex items-center gap-1.5"><i class="fas fa-file-invoice text-slate-500"></i> Cetak
                        nota/invoice.</span>
                    <span class="flex items-center gap-1.5"><i class="fas fa-trash text-rose-500"></i> Hapus
                        transaksi.</span>
                </div>
            </div>
        </div>
        ...
        <div
            ...
            <div
                ...
                <a href="{{ route('admin.transaksi.create') }}"
                    class="inline-flex items-center gap-2 bg-primary-600 hover:bg-primary-700 text-white px-4 py-2 rounded-xl text-sm font-semibold transition shadow-sm shadow-primary-500/20">
                                ...
                                <tr class="hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors">
                                    ...
                                    <td class="px-4 py-4">
                                        <div class="font-bold text-slate-900 dark:text-white">
                                            {{ $transaksi->pelanggan?->user?->name ?? 'Umum' }}</div>
                                    ...
                                    <td class="px-4 py-4">
                                        ...
                                        <div class="text-[10px] uppercase tracking-wider text-primary-500 font-bold">
                                    ...
                                    <td class="px-4 py-4 text-center">
                                        @php
                                            ...
                                            $currentClass =
                                                $statusClasses[$transaksi->status] ?? 'bg-rose-50 text-rose-600';
                                        @endphp

                                        <form action="{{ route('admin.transaksi.status', $transaksi->id) }}" method="POST"
                                            class="inline">
                                            @csrf
                                            @method('PUT')
                                            <button type="button"
                                                class="btn-status px-3 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-widest transition-transform hover:scale-105 {{ $currentClass }}">
                                                {{ $transaksi->status }}
                                            </button>
                                        </form>
                                    </td>
                                    <td class="px-4 py-4">
                                        <div class="flex items-center justify-center gap-1.5">
                                            ...
                                            <a href="{{ route('admin.transaksi.show', $transaksi->id) }}"
                                                class="w-8 h-8 flex items-center justify-center rounded-lg bg-slate-100 text-slate-600 hover:bg-slate-600 hover:text-white dark:bg-slate-700 dark:text-slate-300 transition-all"
                                                title="Nota">
