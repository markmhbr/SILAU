    ...
    <div class="max-w-5xl mx-auto space-y-8 animate-fadeIn">
        ...
        <div>
            ...
            <div class="flex items-center gap-4">
                ...
                <span
                    class="px-3 py-1 rounded-full text-[10px] font-black tracking-widest uppercase bg-purple-100 text-purple-600">
                    {{ $user->role }}
                </span>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
            {{-- FORM EMAIL --}}
            <div
                class="bg-white dark:bg-slate-900 rounded-[2.5rem] p-8 md:p-10 shadow-sm border border-slate-200 dark:border-slate-800 relative overflow-hidden">
                ...
                <form action="{{ route('akun.email', ['role' => auth()->user()->role]) }}" method="POST" class="space-y-6">
                    ...
                    @method('PATCH')
                    <div>
                        <label class="block text-xs font-black text-slate-400 uppercase tracking-widest mb-2 ml-1">Nama
                            Admin</label>
                        <input type="text" value="{{ $user->name }}" disabled
                            class="w-full px-6 py-4 rounded-2xl border border-slate-100 bg-slate-50 dark:bg-slate-800/50 dark:border-slate-800 text-slate-400 font-semibold cursor-not-allowed">
                    </div>
                    <div>
                        <label class="block text-xs font-black text-slate-400 uppercase tracking-widest mb-2 ml-1">Email
                            Aktif</label>
                        ...
                        @error('email')
                            <p class="text-rose-500 text-[10px] font-bold mt-2 uppercase tracking-tight">{{ $message }}</p>
                        @enderror
                    </div>
                    <button type="submit"
                        class="w-full bg-blue-600 hover:bg-blue-700 text-white px-8 py-4 rounded-2xl font-black text-sm transition-all">
                        Simpan Email Baru
                    </button>
                </form>
            </div>
            ...
            <div
                ...
                <form action="{{ route('akun.password', ['role' => auth()->user()->role]) }}" method="POST"
                    class="space-y-4">
                    @csrf
                    @method('PATCH')

                    @php $fields = [['name' => 'current_password', 'label' => 'Password Lama', 'placeholder' => '••••••••'], ['name' => 'password', 'label' => 'Password Baru', 'placeholder' => 'Minimal 8 karakter'], ['name' => 'password_confirmation', 'label' => 'Ulangi Password', 'placeholder' => 'Ketik ulang password']]; @endphp

                    @foreach ($fields as $field)
                        <div>
                            <label
                                class="block text-xs font-black text-slate-400 uppercase tracking-widest mb-2 ml-1">{{ $field['label'] }}</label>
                            <div class="relative group">
                                ...
                                <button type="button"
                                    class="toggle-password absolute right-5 top-1/2 -translate-y-1/2 text-slate-400 hover:text-rose-500 transition">
                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    ...
                    <div class="pt-2">
                        <button type="submit"
                            class="w-full bg-slate-900 dark:bg-rose-500 hover:bg-black text-white px-8 py-4 rounded-2xl font-black text-sm transition-all shadow-lg shadow-rose-500/20">
                            Update Keamanan
                        </button>
                    </div>
                </form>
